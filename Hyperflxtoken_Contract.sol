...
contract Main is ERC20, Ownable, SafeToken {
    ...
    constructor(
        ...
        address _DEXV2Pair = IDexFactory(_dexRouter.factory()).createPair(
            address(this),
            _dexRouter.WETH()
        );
        ...
        } else if (DispatchSupply == InitialSupply) {
    ...
    function GetExclusions(
        address account
    ) public view returns (bool MaxTx, bool Fees, bool Whitelist) {
        return (
    ...
    function GetDelay()
        public
        view
        returns (bool delayoption, uint256 mintime)
    {
        return (DelayOption, MinTime);
    }

    function GetContractAddresses()
        public
        view
        returns (address marketing, address Dead, address LP)
    {
        return (
    ...
    {
        return (JeetsFee, JeetsBurn, VmaxDiv, VminDiv, MaxJeetsFee);
    }

    function GetContractBalance() external view returns (uint256 marketingETH) {
        return (marketingETHPortion);
    }
    ...
    function getLiquidityUnlockTime()
        public
        view
        returns (uint256 Days, uint256 Hours, uint256 Minutes, uint256 Seconds)
    {
        if (block.timestamp < _liquidityUnlockTime) {
            ...
            Hours =
                (_liquidityUnlockTime - block.timestamp - Days * 86400) /
                3600;
            Minutes =
    ...
    function buyBackBurn(uint256 amountInWei) external {
        ...
        DEXV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{
            value: amountInWei
        }(0, path, address(DeadWallet), block.timestamp);
    ...
    function SetLimits(
        ...
        uint256 supply = totalSupply();
        require(
            _maxWallet * 10 ** decimal >= supply / 100 &&
                _maxWallet * 10 ** decimal <= supply,
            "MawWallet must be between totalsupply and 1% of totalsupply"
        );
        require(
            _maxSell * 10 ** decimal >= supply / 1000 &&
                _maxSell * 10 ** decimal <= supply,
            "MawSell must be between totalsupply and 0.1% of totalsupply"
        );
        require(
            _minswap * 10 ** decimal >= supply / 10000 &&
                _minswap <= _swapmax / 2,
            "MinSwap must be between maxswap/2 and 0.01% of totalsupply"
        );
        require(
            MaxTax >= 1 && MaxTax <= 25,
            "Max Tax must be updated to between 1 and 25 percent"
        );
        require(
            _swapmax >= _minswap * 2 && _swapmax * 10 ** decimal <= supply,
            "MaxSwap must be between totalsupply and SwapMin x 2"
        );
    ...
    function SetTaxes(
        uint256 newBuyTax,
        uint256 wallet2walletfee,
        uint256 newLiquidityTax,
        uint256 newBurnTax,
        uint256 newMarketingTax
    ) external onlyOwner {
        require(
            newBuyTax <= MaxTaxes && newBuyTax >= newBurnTax,
            "Total Tax can't exceed MaxTaxes. or be lower than burn tax"
        );
        ...
        require(
            newMarketingTax >= 0 &&
                ...
                newLiquidityTax >= 0 &&
                newBurnTax >= 0,
            "No tax can be negative"
        );
        if (wallet2walletfee != 0) {
            require(
                wallet2walletfee >= _BurnFee && wallet2walletfee <= MaxTaxes,
                "Wallet 2 Wallet Tax must be updated to between burn tax and 25 percent"
            );
        }
    ...
    function setAutomatedMarketMakerPair(
        ...
        require(
            pair != DEXV2Pair,
            "The Market pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function ExtendLockTime(
        ...
        if (_liquidityUnlockTime < block.timestamp)
    ...
    function Launch(
        ...
        if (address(this).balance == 0) {
            ...
            uint256 lockTimeInSeconds = lockTimeInDays *
                86400 +
                lockTimeInHours *
                3600;
    ...
    function SetJeetsTax(
        ...
        require(msg.sender == safeManager);
        require(
            vmaxdiv >= 10 && vmaxdiv <= 40,
            "cannot set Vmax outside 10%/40% ratio"
        );
        require(
            vmindiv >= 1 && vmindiv <= 10,
            "cannot set Vmin outside 1%/10% ratio"
        );
        require(
            maxjeetsfee >= 1 && maxjeetsfee <= 20,
            "max jeets fee must be betwwen 1% and 20%"
        );
        JeetsFee = jeetsfee;
        JeetsBurn = jeetsburn;
        VmaxDiv = vmaxdiv;
    ...
    function takeFee(
        ...
        if (automatedMarketMakerPairs[from]) {
            ...
            // sell tax applied if sell
            if (JeetsFee && !_isWhitelisted[from]) {
                // Jeets extra Fee against massive dumpers
    ...
    function _transfer(
        ...
        if (!tradingEnabled) {
            require(_isWhitelisted[from], "Trading not allowed yet");
        }
        ...
        if (contractTokenBalance >= MaxTokenToSwap) {
            contractTokenBalance = MaxTokenToSwap;
        }
        ...
        if (to == DeadWallet) {
            super._burn(from, amountToSend);
        }
        // if destination address is Deadwallet, burn amount
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private {
        ...
        uint256 half = (contractTokenBalance * _LiquidityFee) /
            2 /
            (_SellFee - _BurnFee);

        uint256 swapTokens = ((contractTokenBalance * NoRewardFees) /
            (_SellFee - _BurnFee)) - half;
        swapTokensForETH(swapTokens);
        uint256 ETHBalance = address(this).balance - initialBalance;

        uint256 liquidityETHPortion = ((ETHBalance * _LiquidityFee) / 2) /
            (NoRewardFees - (_LiquidityFee / 2));
        marketingETHPortion +=
            (ETHBalance * _MarketingFee) /
            (NoRewardFees - (_LiquidityFee / 2));
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ETHAmount) private {
        uint256 Allowance = allowance(address(this), address(DEXV2Router)) +
            tokenAmount;
